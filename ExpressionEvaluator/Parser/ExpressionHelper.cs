...
namespace ExpressionEvaluator.Parser
{
    internal class ExpressionHelper
    {
        ...
        {
            // remove leading dot
            //membername = membername.Substring(1);1470
            ...
            else
            {
                ...
                if (!isDynamic)
                {
                    var prop = type.GetPublicProperty(membername);
                    if (prop != null)
                    {
            ...
            else
            {
                ...
                var propertyInfo = type.GetPublicProperty(membername);
        ...
        public static Expression ParseIntLiteral(string token)
        {
            ...
            if (m.Success)
            {
                ...
                else
                {
                    ...
                    else if (uint.TryParse(token, NumberStyles.Integer, CultureInfo.InvariantCulture, out uint uintval))
                    {
                        val = uintval;
                        ntype = typeof(UInt32);
                    }
                    else if (long.TryParse(token, NumberStyles.Integer, CultureInfo.InvariantCulture, out long longval))
                    {
                        val = longval;
                        ntype = typeof(Int64);
                    }
                    else if (ulong.TryParse(token, NumberStyles.Integer, CultureInfo.InvariantCulture, out ulong ulongval))
                    {
                        val = ulongval;
                        ntype = typeof(UInt64);
                    }
